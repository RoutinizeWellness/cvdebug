  ...
  if (!resume) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-slate-400">Loading ATS view...</div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-[#0F172A] overflow-hidden">
      ...
      <header className="h-12 md:h-16 flex items-center justify-between px-3 md:px-6 border-b border-slate-700 bg-slate-800 shrink-0 z-20">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex items-center gap-2 text-sm text-slate-400">
            <button onClick={onBack} className="hover:text-white transition-colors">Jobs</button>
            <span className="material-symbols-outlined text-xs">chevron_right</span>
            <a className="hover:text-white transition-colors" href="#">Engineering</a>
            <span className="material-symbols-outlined text-xs">chevron_right</span>
            <span className="text-white font-medium">Senior Frontend Dev #4092</span>
          </nav>
        </div>
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 pl-6 border-l border-slate-700">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-medium text-white">Recruiter Admin</p>
              <p className="text-xs text-slate-400">View as: Machine</p>
            </div>
            <div
              className="bg-center bg-no-repeat bg-cover rounded-full size-9 border border-slate-700"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-80 flex flex-col border-r border-slate-700 bg-[#0F172A] shrink-0">
          {/* Search & Filter */}
          <div className="p-4 border-b border-slate-700 bg-[#0F172A]/50 backdrop-blur-sm sticky top-0 z-10">
            ...
            <div className="relative">
              ...
              <input
                className="w-full bg-slate-800 border border-slate-700 rounded-lg py-2 pl-10 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all"
                placeholder="Search by keyword..."
                type="text"
              />
            </div>
            <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
              <span className="px-2 py-1 bg-slate-800 border border-slate-700 rounded text-xs text-slate-300 whitespace-nowrap cursor-pointer hover:border-slate-500">
                Unreviewed
              </span>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#0F172A] relative">
          ...
          <div className="sticky top-0 z-20 bg-[#0F172A]/90 backdrop-blur-md border-b border-slate-700 px-8 py-4 flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white mb-1">{currentCandidate.name}</h1>
            ...
            <div className="flex bg-slate-800 rounded-lg p-1 border border-slate-700">
              <button
                ...
                className={`px-4 py-1.5 rounded text-xs font-medium transition-colors ${
                  activeView === "parsed" ? "bg-primary text-white shadow-sm" : "text-slate-400 hover:text-white"
                }`}
              >
                Parsed View
              </button>
              <button
                ...
                className={`px-4 py-1.5 rounded text-xs font-medium transition-colors ${
                  activeView === "raw" ? "bg-primary text-white shadow-sm" : "text-slate-400 hover:text-white"
                }`}
              >
                Raw Text
              </button>
              <button
                ...
                className={`px-4 py-1.5 rounded text-xs font-medium transition-colors ${
                  activeView === "pdf" ? "bg-primary text-white shadow-sm" : "text-slate-400 hover:text-white"
                }`}
              >
                Original PDF
              </button>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-lg">
                <div className="bg-gradient-to-r from-slate-800 to-slate-700 p-4 border-b border-slate-700 flex items-center justify-between">
                  ...
                  <span className="text-xs font-mono text-slate-400">Scan ID: #9923-AX</span>
                </div>
                <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                  {formatIssues.length > 0 && (
                    <div className="flex items-start gap-3 p-3 rounded bg-red-500/10 border border-red-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-slate-300 text-sm">{formatIssues[0]?.issue || "Issues detected in document"}</p>
                      </div>
                    </div>
                  )}
                  <div className="flex items-start gap-3 p-3 rounded bg-emerald-500/10 border border-emerald-500/20">
                    ...
                    <div>
                      ...
                      <p className="text-slate-300 text-sm">PDF Structure is valid and machine-readable.</p>
                    </div>
                  </div>
                  {missingKeywords.length > 0 && (
                    <div className="flex items-start gap-3 p-3 rounded bg-amber-500/10 border border-amber-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-slate-300 text-sm">Missing {missingKeywords.length} critical keywords from JD.</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 shadow-sm">
                  <p className="text-slate-300 leading-relaxed">
                    {resume.analysis ? (
                      ...
                      <>
                        Senior professional with <span className="text-white bg-primary/20 px-1 rounded font-medium">excellent experience</span> in the field.
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 shadow-sm">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {missingKeywords.slice(0, 3).map((item: any, i: number) => (
                      <span key={i} className="inline-flex items-center px-2.5 py-1 rounded bg-slate-700/50 text-slate-300 border border-slate-600 text-sm font-mono">
              ...
              <section className="group relative pb-8">
                ...
                <div className="space-y-4">
                  <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 shadow-sm relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-3">
                      <span className="text-xs font-mono text-slate-500 bg-slate-700 px-2 py-1 rounded">Recent</span>
                    </div>
                    <h5 className="text-white font-semibold text-lg">Professional Experience</h5>
                    <p className="text-primary text-sm font-medium mb-4">Various Companies • Multiple Years</p>
                    <ul className="list-disc list-outside ml-4 space-y-2 text-slate-300 text-sm marker:text-slate-600">
        ...
        <aside className="w-[340px] flex flex-col border-l border-slate-700 bg-slate-800 shrink-0 overflow-y-auto">
          {/* Gauge Section */}
          <div className="p-6 border-b border-slate-700 text-center">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-6">Job Fit Score</h2>
          ...
          <div className="p-4 space-y-4 flex-1">
            ...
            <div className="bg-[#0F172A] border border-slate-700 rounded-lg p-4">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-sm font-medium text-white">Seniority Level</h3>
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-500 mb-2 font-mono uppercase">
                <span className={seniorityLevel === 0 ? "text-white font-bold" : ""}>Jr</span>
                <span className={seniorityLevel === 1 ? "text-white font-bold" : ""}>Mid</span>
                <span className={seniorityLevel === 2 ? "text-white font-bold" : ""}>Senior</span>
                <span className={seniorityLevel === 3 ? "text-white font-bold" : ""}>Lead</span>
              </div>
            ...
            {/* Gap Analysis */}
            <div className="bg-[#0F172A] border border-slate-700 rounded-lg p-4">
              <h3 className="text-sm font-medium text-white mb-3">Technical Gap Analysis</h3>
          ...
          <div className="p-4 border-t border-slate-700 bg-slate-800 sticky bottom-0 z-10">
            <div className="grid grid-cols-2 gap-3 mb-3">
              <button className="flex items-center justify-center gap-2 py-2.5 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-700 hover:text-white transition-all text-sm font-medium">
            ...
            <button className="w-full flex items-center justify-center gap-2 py-3 rounded-lg bg-primary hover:bg-blue-600 text-white shadow-[0_0_15px_rgba(59,130,246,0.3)] transition-all text-sm font-bold">
