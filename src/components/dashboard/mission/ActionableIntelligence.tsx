...
export function ActionableIntelligence({ issues }: ActionableIntelligenceProps) {
  return (
    <motion.div 
      className="glass-panel rounded-xl flex flex-col neon-glow bg-white/70 backdrop-blur-xl border border-slate-200/50"
      ...
      <div className="p-4 flex flex-col gap-3">
        {issues.length > 0 ? (
          issues.map((issue, idx) => (
            <div key={idx} className="flex items-start gap-3 p-3 rounded bg-slate-50/50 border border-slate-200/50 hover:border-slate-600 transition-colors">
              ...
              <div>
                <p className="text-sm text-slate-700 font-medium">{issue.title}</p>
