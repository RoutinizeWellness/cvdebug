...
export function ActionableIntelligence({ issues }: ActionableIntelligenceProps) {
  return (
    <motion.div 
      className="glass-panel rounded-xl flex flex-col neon-glow bg-[#FFFFFF]/70 backdrop-blur-xl border border-[#E2E8F0]/50"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.3 }}
    >
      <div className="p-5 border-b border-[#E2E8F0]/50">
      ...
      <div className="p-4 flex flex-col gap-3">
        {issues.length > 0 ? (
          issues.map((issue, idx) => (
            <div key={idx} className="flex items-start gap-3 p-3 rounded bg-[#F8FAFC]/50 border border-[#E2E8F0]/50 hover:border-slate-600 transition-colors">
              ...
              <div>
                <p className="text-sm text-[#475569] font-medium">{issue.title}</p>
                <p className="text-xs text-[#64748B] mt-1">{issue.description}</p>
              </div>
            </div>
          ))
        ) : (
