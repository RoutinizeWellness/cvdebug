...
export function BulletPointSniper({ 
  ...
  return (
    <motion.div 
      className="glass-panel rounded-xl flex flex-col h-[600px] overflow-hidden neon-glow bg-[#FFFFFF]/70 backdrop-blur-xl border border-[#E2E8F0]/50"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.2 }}
    >
      {/* Header */}
      <div className="p-5 border-b border-[#E2E8F0]/50 bg-[#F8FAFC]/30">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
          <TabsList className="grid w-full grid-cols-2 bg-[#FFFFFF]/50">
            <TabsTrigger value="missing" className="data-[state=active]:bg-rose-500/20 data-[state=active]:text-rose-400 text-xs">
              Missing ({missingKeywords.length})
            </TabsTrigger>
            <TabsTrigger value="found" className="data-[state=active]:bg-[#22C55E]/20 data-[state=active]:text-emerald-400 text-xs">
                    ...
                    >
                      <div className="p-3 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`p-1.5 rounded ${isFixed ? "bg-yellow-500/20 text-[#F59E0B]" : "bg-rose-900/50 text-rose-400"}`}>
                            {isFixed ? <Check className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}
                          </div>
                          <div className="flex flex-col">
                            <span className={`text-sm font-medium ${isFixed ? "text-yellow-200" : "text-[#0F172A]"}`}>
                              {keyword.name}
                            </span>
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button 
                              ...
                              className={`h-8 text-xs font-semibold gap-2 ${
                                !isPremium 
                                  ? "bg-[#F8FAFC] text-[#64748B] hover:bg-slate-700" 
                                  : "bg-rose-500/10 text-rose-400 border border-rose-500/30 hover:bg-rose-500 hover:text-[#0F172A] shadow-[0_0_10px_rgba(244,63,94,0.1)]"
                              }`}
                            >
                              {isSniping ? (
                                <>
                      ...
                      {showingContext && !bullets && (
                        <div className="border-t border-[#E2E8F0] p-3 bg-[#FFFFFF]/50 space-y-2">
                          ...
                          <Textarea
                            ...
                            className="min-h-[60px] bg-slate-950 border-[#E2E8F0] text-[#475569] text-xs"
                          />
                          <div className="flex gap-2">
                            <Button
                              size="sm"
                        ...
                        {bullets && (
                          ...
                          >
                            <div className="p-3 space-y-3">
                              ...
                              {/* Performance Version */}
                              {bullets.performance && (
                                <div className="group relative p-3 rounded bg-[#F8FAFC]/50 border border-[#E2E8F0] hover:border-primary/50 transition-colors">
                                  <div className="flex justify-between gap-3">
                                    ...
                                    <Button
                                      size="icon"
                                      variant="ghost"
                                      className="h-6 w-6 shrink-0 text-[#64748B] hover:text-[#0F172A] hover:bg-slate-700"
            ...
            {activeTab === "found" && (
              <>
                {matchedKeywords.map((keyword, idx) => (
                  <div key={`matched-${idx}`} className="flex items-center justify-between p-3 rounded-lg bg-[#F8FAFC]/30 border border-[#E2E8F0]/50">
                    <div className="flex items-center gap-3">
                      <div className="p-1.5 rounded bg-[#22C55E]/10 text-emerald-400">
      ...
      {/* Footer Action */}
      <div className="p-4 border-t border-[#E2E8F0]/50 bg-[#F8FAFC]/30 backdrop-blur">
        <Button 
          onClick={onUpdateResume}
          className="w-full bg-primary hover:bg-primary/90 text-[#0F172A] font-medium shadow-lg shadow-primary/25 flex items-center justify-center gap-2"
        >
          <ArrowUpCircle className="h-4 w-4" />
