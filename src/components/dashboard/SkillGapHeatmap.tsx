...
export function SkillGapHeatmap({ foundKeywords, missingKeywords }: SkillGapHeatmapProps) {
  ...
  return (
    <div className="glass-card rounded-lg p-6 flex flex-col gap-4 relative overflow-hidden group">
      ...
      >
        {foundKeywords.map((kw: string, i: number) => (
          <motion.span
            key={i}
            variants={item}
            className="px-3 py-1.5 rounded-full bg-white text-stone-900 text-xs font-bold flex items-center gap-1"
          >
        ...
        {missingKeywords.length > 2 && (
          <>
            <span className="px-3 py-1.5 rounded-full bg-stone-800 text-stone-500 text-xs font-medium blur-[2px]">
              Docker
            </span>
            <span className="px-3 py-1.5 rounded-full bg-stone-800 text-stone-500 text-xs font-medium blur-[2px]">
              Kubernetes
      ...
      {missingKeywords.length > 2 && (
        <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[#050505] via-[#050505]/90 to-transparent flex flex-col items-center justify-end pb-6">
