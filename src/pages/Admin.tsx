...
export default function AdminPage() {
  ...
  const handleFixReported = async () => {
    ...
    try {
      ...
      toast.success("Reported Users Fix Complete", {
        description: <pre className="mt-2 w-[340px] rounded-md bg-slate-950 p-4 overflow-auto text-xs text-slate-900">{result}</pre>,
        duration: 10000,
      });
    } catch (error) {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-slate-50">
      {/* Sidebar Navigation */}
      <aside className={`w-64 flex-shrink-0 flex-col border-r border-slate-200 bg-white shadow-sm ${sidebarOpen ? 'flex' : 'hidden'} md:flex`}>
        <div className="h-16 flex items-center px-6 border-b border-slate-200">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-r from-cyan-600 to-teal-600 flex items-center justify-center mr-3 shadow-lg">
            <ShieldAlert className="text-slate-900 h-5 w-5" />
          </div>
          <div>
            <h1 className="text-slate-900 font-display font-bold text-2xl leading-tight tracking-tight">CVDebug</h1>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 flex flex-col gap-1">
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "dashboard" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "users" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "premium" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "analytics" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "logs" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
            <Terminal className={`h-5 w-5 ${currentView === "logs" ? "text-primary" : "text-slate-500"}`} />
            <span className="text-sm font-medium">Logs & API</span>
          </button>

          <div className="mt-8 px-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Settings</div>
          <button
            onClick={() => setCurrentView("settings")}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "settings" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${currentView === "billing" ? "bg-primary/10 border border-primary/20 text-primary" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}
          >
        ...
        <div className="p-4 border-t border-slate-200">
          <button
            ...
            className="flex items-center gap-3 w-full px-3 py-2 text-slate-600 hover:text-red-600 transition-colors rounded-lg hover:bg-red-50"
          >
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50 relative">
        ...
        <header className="h-16 border-b border-slate-200 flex items-center justify-between px-6 bg-white shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-slate-900 text-xl font-display font-bold">Admin Dashboard</h2>
            </div>
          </div>
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              className="bg-gradient-to-r from-cyan-600 to-teal-600 text-slate-900 shadow-sm hover:shadow-md"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 z-0 bg-slate-50">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    <p className="text-slate-600 text-sm font-medium mb-1">Free Users</p>
                    <div className="flex items-baseline gap-2">
                      <h3 className="text-2xl font-display font-bold text-slate-900">{stats?.free || 0}</h3>
                    </div>
                    <div className="w-full bg-slate-200 h-1 mt-4 rounded-full overflow-hidden">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.1 }}
                    className="glass-panel p-6 rounded-xl border border-slate-200/50 hover:border-primary/30 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-display font-bold text-slate-900">Create New User</h3>
                        <p className="text-sm text-slate-600">Add users directly to Clerk + Convex</p>
                      </div>
                    ...
                    <Button
                      ...
                      className="w-full bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 text-slate-900 shadow-lg shadow-emerald-900/30"
                    >
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 min-h-[500px]">
                  {/* User Table */}
                  <div className="xl:col-span-2 glass-panel rounded-xl border border-slate-200/50 flex flex-col overflow-hidden">
                    <div className="p-5 border-b border-slate-200/50 flex flex-wrap gap-4 justify-between items-center bg-slate-100/20">
                      <div>
                        <h3 className="text-lg font-display font-bold text-slate-900">Recent Users</h3>
                        <p className="text-slate-600 text-sm">Managing {stats?.total || 0} total accounts</p>
                      </div>
                      ...
                      <table className="w-full text-left text-sm text-slate-600">
                        ...
                        <tbody className="divide-y divide-slate-800/50">
                          {filteredUsers.slice(0, 5).map((userData: any) => (
                            <tr key={userData._id} className="hover:bg-slate-100/30 transition-colors group">
                              <td className="px-6 py-3">
                                <div className="flex items-center gap-3">
                                  <div className="w-9 h-9 rounded-full bg-slate-200 flex items-center justify-center text-slate-900 font-bold text-xs border border-slate-300">
                  ...
                  {/* Live Logs Terminal */}
                  <div className="glass-panel rounded-xl border border-slate-200/50 flex flex-col overflow-hidden h-full shadow-lg">
                    <div className="p-3 bg-slate-100 border-b border-slate-200 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Terminal className="h-4 w-4 text-slate-600" />
                        <span className="font-mono text-xs font-bold text-slate-700 uppercase tracking-wider">Live Logs</span>
                      </div>
                    ...
                    <div className="flex-1 bg-slate-50 border-slate-200 p-4 font-mono text-xs overflow-y-auto flex flex-col gap-3 font-medium">
                      <div className="flex gap-2 opacity-60">
                        <span className="text-slate-600 min-w-[60px]">10:41:02</span>
                        <div className="flex-1 text-slate-700">
                          <span className="text-slate-600">[INFO]</span> System initialization complete.
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-slate-600 min-w-[60px]">10:42:15</span>
                        <div className="flex-1 text-slate-700">
                          <span className="text-emerald-600">[200 OK]</span> OpenAI API - Resume Analysis <span className="text-slate-500">#492a</span>
                        </div>
                      </div>
                      <div className="flex gap-2 border-l-2 border-cyan-500/50 pl-2 bg-cyan-500/5 py-1 rounded-r">
                        <span className="text-slate-600 min-w-[52px]">10:43:12</span>
                        <div className="flex-1 text-cyan-700">
                          <span className="text-cyan-600 font-bold">[STRIPE]</span> Payment Succeeded - €24.99
                          <span className="text-slate-500 text-[10px] block mt-1">user: {user.email}</span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-slate-600 min-w-[60px]">10:44:05</span>
                        <div className="flex-1 text-slate-700">
                          <span className="text-amber-600">[WARN]</span> High latency detected on worker-04
                        </div>
                      </div>
            ...
            {currentView === "analytics" && (
              <div className="flex items-center justify-center h-[400px] text-slate-600">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-bold text-slate-900">Analytics Module</h3>
      ...
      <Dialog open={!!editingUser} onOpenChange={(open) => !open && setEditingUser(null)}>
        <DialogContent className="bg-white border-slate-200 text-slate-900">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="credits" className="text-right text-slate-700">Credits</Label>
              <Input
                id="credits"
                type="number"
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingUser(null)} className="border-slate-200">Cancel</Button>
      ...
      <Dialog open={showCreateUserDialog} onOpenChange={setShowCreateUserDialog}>
        <DialogContent className="bg-white border-slate-200 text-slate-900">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="firstName" className="text-right text-slate-700">First Name</Label>
              <Input
                id="firstName"
                type="text"
                placeholder="John"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="lastName" className="text-right text-slate-700">Last Name</Label>
              <Input
                id="lastName"
                type="text"
                placeholder="Doe"
