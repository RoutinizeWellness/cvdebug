...
const filePath = '/home/daytona/codebase/src/lib/i18n.ts';

// Read the file
let content = fs.readFileSync(filePath, 'utf8');

// Helper function to find and replace a specific locale section
function replaceSectionForLocale(content, localeName, sectionName, newSectionContent) {
  // Find the locale declaration
  const localePattern = new RegExp(`'${localeName}':\\s*\\{`, 'g');
  const matches = [...content.matchAll(localePattern)];

  if (matches.length === 0) {
    console.log(`Locale '${localeName}' not found`);
    return content;
  }

  const localeStart = matches[0].index;

  // Find the section within this locale
  const sectionPattern = new RegExp(`(\\s+)${sectionName}:\\s*\\{`, 'g');
  let sectionMatch;
  let found = false;

  while ((sectionMatch = sectionPattern.exec(content)) !== null) {
    if (sectionMatch.index > localeStart) {
      // Found a section after the locale start
      ...
      // Now find the comma after the closing brace
  ...
  if (!found) {
    console.log(`Section '${sectionName}' not found in locale '${localeName}'`);
  } else {
    console.log(`Updated '${sectionName}' for locale '${localeName}'`);
  }
...
console.log('Note: This is a test run. Uncomment the fs.writeFileSync line to apply changes.');

// Update en-US sections
content = replaceSectionForLocale(content, 'en-US', 'landing', enUsTranslations.landing);
content = replaceSectionForLocale(content, 'en-US', 'onboarding', enUsTranslations.onboarding);
content = replaceSectionForLocale(content, 'en-US', 'pricingPage', enUsTranslations.pricingPage);
content = replaceSectionForLocale(content, 'en-US', 'modals', enUsTranslations.modals);
